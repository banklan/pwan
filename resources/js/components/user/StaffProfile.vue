<template>
    <v-container>
            ...
            <v-col cols="12" md="5">
                <v-card light raised outlined elevation="4" min-height="400" class="scroll" width="100%">
                    <template v-if="!changePic">
                        <v-img v-if="authUser.picture" :src="`/images/profiles/${ authUser.picture }`" width="100%" height="350" transition="scale-transition"></v-img>
                        ...
                        <v-card-actions class="justify-center my-3">
                            <v-btn text color="blue darken-3" class="mt-2 mb-n4" @click="changePic = true">Change Profile Picture</v-btn>
                        </v-card-actions>
                    </template>
                    <template v-if="changePic">
                        ...
                        <v-card-text>
                            <table class="table table-hover table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Last Updated: </th>
                                        <td>{{ authUser.updated_at | moment('Do MMM, YYYY - H:mma') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </v-card-text>
                        <v-card-actions class="justify-center mt-n3 pb-6">
                            <v-btn icon color="admin" @click="openEditForm"><i class="uil uil-edit-alt"></i></v-btn>
                        </v-card-actions>
                    </template>
                    <template v-else>
        ...
        <v-snackbar v-model="updateProfileFailed" :timeout="6000" top color="red darken-1 white--text">
            There was an error while trying to update your profile picture. Please try again while ensuring you are uploading a valid file format.
            <v-btn text color="white--text" @click="updateProfileFailed = false">Close</v-btn>
        </v-snackbar>
        <v-snackbar :value="updatedUserProfile" :timeout="3000" top color="green darken-1 white--text">
            Your profile was updated successfully.
            <v-btn text color="white--text" @click="updatedUserProfile = false">Close</v-btn>
        </v-snackbar>
        ...
        <!-- <v-snackbar v-model="profilePicUpdated" :timeout="4000" top color="green darken-1 white--text">
            Your profile picture was updated successfully.
            <v-btn text color="white--text" @click="profilePicUpdated = false">Close</v-btn>
        </v-snackbar> -->
    </v-container>
</template>
...
export default {
    ...
    computed: {
        ...
        updatedUserProfile(){
            return this.$store.getters.updatedUserProfile
        }
    },
    methods: {
        openEditForm(){
        ...
        updateProfile(){
            this.$validator.validateAll().then((isValid) => {
                if(isValid) {
                    ...
                    axios.post(this.api + '/auth/update_user_profile', {
                        user: this.edit
        ...
        uploadImg(){
            if(this.image !== null){
                ...
                axios.post(this.api + `/auth/update_user_profile_picture`, form, this.authHeaders)
                .then((res) => {
                    this.isUpdating = false
                    this.$store.commit('updatedUserProfile', res.data)
