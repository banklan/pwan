            ...
            <template v-if="!userCreated">
                <v-col cols="12" md="6">
                    <v-card light raised elevation="12" min-height="350" class="mx-auto my-10">
                        <v-card-title class="primary white--text justify-center headline font-weight-regular">Register As A Staff</v-card-title>
                        <v-card-text class="body-1 mt-4 px-7">
                            ...
                            <div class="my-3 subtitle-2 justify-center info lighten-3 pa-3 font-weight-bold">Create a password of between 5 & 20 alphanumeric (mix of numbers,alphabets, characters) characters.</div>
                        ...
                        <v-card-actions class="justify-center mt-n4 pb-5 login_link">
                            Already a staff? <router-link to="/login" class="primary--text">&nbsp; Login</router-link>
                        </v-card-actions>
                    </v-card>
                </v-col>
            </template>
            <template v-else>
                <v-col cols="12" md="8">
                    <v-alert type="success" border="left" class="mt-10 pa-4">
                        Your registration was successful. An email has been sent to your mailbox <strong>{{ user.email }}</strong> for email verification.
                    </v-alert>
                </v-col>
            </template>
        </v-row>
    ...
    methods: {
        register(){
            this.$validator.validateAll().then((isValid) => {
                if (isValid) {
                    this.isLoading = true
                    axios.post(this.api + `/staff_register`, {
                        user: this.user
                    }).then((res) => {
