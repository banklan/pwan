    ...
    <v-container>
        <v-row justify="center">
            <v-col cols="10" md="6" offset-md="4">
                <admin-search model="NewOffer" searchFor="New offers"/>
            </v-col>
        </v-row>
        <v-row class="mt-2 pr-5" justify="center" >
            <v-col cols="12" md="11">
                ...
                <v-card v-else light raised elevation="8" min-height="100" class="scroll">
                    <v-card-title class="primary white--text subtitle-1 justify-center">New Offers List <span v-if="offers.length > 0"><v-chip color="primary lighten-2 ml-2">{{ offers.length }}</v-chip></span></v-card-title>
                    <v-card-text>
                        <table class="table table-hover table-striped" v-if="offers.length > 0">
                            ...
                            <tbody class="offers_list">
                                <tr v-for="offer in offers" :key="offer.id" :class="!offer.is_approved ? 'warning--text' : ''">
                                    ...
                                    <td><v-btn icon color="primary"><i class="uil uil-edit"></i></v-btn><v-btn icon color="red darken-2" @click="confirmDel"><i class="uil uil-trash-alt"></i></v-btn></td>
                                </tr>
                            </tbody>
                        </table>
                        <v-alert class="mt-5" type="info" border="left" v-else>
                            There are no offers to display at the moment.
                        </v-alert>
                    </v-card-text>
        ...
        <v-dialog v-model="confirmDelDialog" max-width="450">
            <v-card min-height="100">
                ...
                <v-card-actions class="pb-8 justify-center">
                    ...
                    <v-btn dark width="40%" color="primary" :loading="isDeleting" @click="deleteOffer">Yes, Delete</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <v-snackbar v-model="offerDeleted" :timeout="4000" top dark color="green darken-2">
            A new offer was deleted successfully.
            <v-btn text color="white--text" @click="offerDeleted = false">Close</v-btn>
        </v-snackbar>
        <v-snackbar :value="adminDeletedNewOffer" :timeout="4000" top dark color="green darken-2">
            A new offer was deleted successfully.
            <v-btn text color="white--text" @click="adminDeletedNewOffer">Close</v-btn>
        </v-snackbar>
    </v-container>
</template>
...
export default {
    ...
    computed:{
        ...
        adminDeletedNewOffer(){
            return this.$store.getters.adminDeletedNewOffer
        }
    },
    methods: {
        getOffers(pag){
            this.isLoading = true
            pag = pag || `${this.api}/auth-admin/get_paginated_offers`
        ...
        confirmDel(post, i){
            ...
            this.offerToDel = post
            this.offerToDelIndex = i
        },
        deleteOffer(){
            this.isDeleting = true
            axios.post(this.api + `/auth-admin/delete_offer/${this.offerToDel}`, {}, this.adminHeaders)
            .then((res) => {
                this.offers.splice(this.offerToDelIndex, 1)
