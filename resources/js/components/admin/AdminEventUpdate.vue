            ...
            this.$validator.validateAll().then((isValid) => {
                if(isValid) {
                    if(this.event.event_date != '' && this.event.event_time != ''){
                        this.isBusy = true
                        axios.post(this.api + `/auth-admin/update_event/${this.$route.params.id}`, {
                            event: this.event
                        }, this.adminHeaders).then((res) => {
                            this.isBusy = false
                            this.$store.commit('adminUpdatedEvent')
                            this.$router.push({name: 'AdminEventDetail', params: {id: res.data.id}})
