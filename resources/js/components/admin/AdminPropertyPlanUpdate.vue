<template>
    <v-container>
        ...
        <v-row justify="center">
            <v-col cols="12" md="8">
                ...
                <v-card v-else light raised elevation="4" min-height="200" class="mx-auto">
                    <v-card-title class="justify-center subtitle-1 primary white--text">Update Listing Plan</v-card-title>
        ...
        <v-snackbar v-model="updateFailed" :timeout="6000" top dark color="red darken-2">
            There was an error while trying to update the listing plan. Please ensure all fields are validly filled before trying again.
...
export default {
    ...
    methods: {
        getPlan(){
            this.isLoading = true
            axios.get(this.api + `/auth-admin/get_listing_plan/${this.$route.params.id}`, this.adminHeaders)
            .then((res) => {
        ...
        update(){
            this.$validator.validateAll().then((isValid) => {
                if(isValid) {
                    this.isBusy = true
                    axios.post(this.api + `/auth-admin/update_listing_plan/${this.plan.id}`, {
                        plan: this.plan
                    }, this.adminHeaders).then((res) => {
                        this.isBusy = false
                        this.$store.commit('adminUpdatedListingPlan')
                        this.$router.push({name: 'AdminPropertyDetail', params: {id: this.plan.property_listing_id}})
