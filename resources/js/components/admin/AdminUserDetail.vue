<template>
    <v-container>
        ...
        <v-row justify="start" class="mt-5" :class="$vuetify.breakpoint.smAndDown ? 'ml-n3 mr-n5': ''">
            <v-col cols="12" md="8" offset-md="2" v-if="user && !user.authorized_by">
                <v-alert type="warning" border="left">
                    This user has not been authorized, kindly authorize to enable him/her login.
                </v-alert>
            </v-col>
            <v-col cols="12" md="6">
                    ...
                    <template v-if="user.is_verified">
                        ...
                        <v-card-actions class="justify-space-around pb-5">
                            <template v-if="user.authorized_by">
                                ...
                                <v-btn dark text color="admin_a" @click="statusDialog = true">{{ user.status == 1 ? 'Disable' : 'Enable' }}</v-btn>
                            </template>
                            <template v-else>
                                <v-btn color="admin_b" dark small @click="confirmAuthDial = true">Authorize</v-btn>
                            </template>
        ...
        <v-dialog v-model="statusDialog" max-width="450">
            <v-card min-height="150">
                <v-card-title class="subtitle-1 white--text primary justify-center">{{ user && user.status == 0 ? 'Enable' : 'Disable'}} this user?</v-card-title>
        ...
        <v-dialog v-model="confirmAuthDial" max-width="450">
            <v-card min-height="150">
                <v-card-title class="subtitle-1 white--text primary justify-center">Are you sure you want to authorize the creation of this user (staff)?</v-card-title>
                <v-card-actions class="pb-8 mt-8 justify-center">
                    <v-btn text width="40%" color="red darken--2" @click="confirmAuthDial = false">Cancel</v-btn>
                    <v-btn dark width="40%" color="primary" :loading="isConfirmingAuth" @click="authorizeUser">Yes, Authorize</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-snackbar v-model="userWasAuthorized" :timeout="4000" top color="green darken-1 white--text">
            The user account has been authorized. He/She can now login.
            <v-btn text color="white--text" @click="userWasAuthorized = false">Close</v-btn>
        </v-snackbar>
    </v-container>
</template>
...
export default {
    ...
    methods: {
        ...
        getAuthorizer(id){
            axios.get(this.api + `/auth-admin/get_user_authorizer/${id}`, this.adminHeaders)
            .then((res) => {
        ...
        authorizeUser(){
            ...
            axios.post(this.api + `/auth-admin/authorize_user/${this.$route.params.id}`, {}, this.adminHeaders)
            .then((res) => {
