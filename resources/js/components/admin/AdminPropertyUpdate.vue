...
export default {
    ...
    methods: {
        getProp(){
            axios.get(this.api + `/auth-admin/get_property/${this.$route.params.id}`, this.adminHeaders)
            .then((res) => {
                this.prop = res.data
        ...
        updateProp(){
            this.$validator.validateAll().then((isValid) => {
                if(isValid) {
                    this.isBusy = true
                    axios.post(this.api + `/auth-admin/admin_update_property/${this.$route.params.id}`, {
                        property: this.prop
